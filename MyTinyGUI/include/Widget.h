...
public:
    ...
    */
    virtual void update(SDL_Event *e)
    {
        ...
        Vector2D mousePosition=Vector2D(e->motion.x,e->motion.y);
        if(e->button.button==SDL_BUTTON_LEFT&&m_currentFrame!=MOUSE_HOVER)
        {
            m_bRealease=false;
        }
        ...
        if(mousePosition.getX()<(position.getX()+m_width) &&mousePosition.getX()>position.getX()&&mousePosition.getY()<(position.getY()+m_height)&&mousePosition.getY()>position.getY())
        {
            ...
            if(e->button.button==SDL_BUTTON_LEFT&&m_bRealease)
            {
