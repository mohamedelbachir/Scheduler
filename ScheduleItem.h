...
enum tabState {MAIN_STATE,DISPLAY_STATE};
...
public:
    ...
    int VerifiedUserData()
    {
        if(m_algoGenType==ALG_DEFAULT)
        {
            for (size_t i = 0; i < m_processes.size(); i++)
            {
                ...
                {
                    if((row!=i
                            &&m_processes[row].Sid==pId)
                            ||strlen(m_processes[row].Sid)<1
                      )
                    {
    ...
    void AddProcess()
    {
        ...
        n_process_generated++;
        if(isPidChecked)isPidChecked=false;
    }
    ...
    void render()
    {
        ...
        {
            isActive=true;
            //ImGui::Text("This is the tab : %s", nameTab.c_str());
