...
#endif
#include "Screens/ScheduleItem.h"
#include "MainApp.h"
...
void ScheduleTaskTab::parsingScheduleElement(TiXmlElement *elmt)
{
    ...
    for (TiXmlElement *e = elmt->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
    {
        ...
        if (strcmp(e->Value(), "Priority") == 0)
        {
            e->Attribute("value", (int *)&withPriority);
        }
        ...
        if (strcmp(e->Value(), "Process") == 0)
        {
            ...
            if (withPriority)
            {
                e->Attribute("priority", (int *)&p.priority);
            }
...
bool ScheduleTaskTab::saveFile()
{
...
#if __unix__
        sprintf(pathSavedFile, "%s%s", pathSavedFile, APP_EXTENSION_FILE);
#endif
#endif
    }
    if (pathSavedFile != NULL)
    {
        ...
        p_element = new TiXmlElement("Priority");
        p_element->SetAttribute("value", (bool)withPriority);
        parameter->LinkEndChild(p_element);
        ...
        for (size_t i = 0; i < m_processes.size(); i++)
        {
            ...
            if (withPriority)
                data->SetAttribute("priority", m_processes[i].priority);
