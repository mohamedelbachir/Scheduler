...
void ScheduleTaskTab::displayState()
{
    ...
    ImGui::BeginChild("left panel", ImVec2(250, 0), false, ImGuiWindowFlags_NoSavedSettings);
    ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1);
    ...
    ImGui::SameLine();
    ImGui::BeginGroup();
    ImGui::BeginChild("parent view", ImVec2(ImGui::GetContentRegionAvail().x, (m_showTOP || m_showSynopsis) ? ImGui::GetContentRegionAvail().y - ImGui::GetContentRegionAvail().y / 3 : ImGui::GetContentRegionAvail().y), false, ImGuiWindowFlags_NoSavedSettings);
    ImGui::BeginChild("content view", ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y), false, ImGuiWindowFlags_NoSavedSettings);
    ...
    if (m_showLegend)
    {
        ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoMove;
        ...
        if (ImGui::Begin(label, &m_showLegend, window_flags))
        {
            ...
            static const ImGuiColorEditFlags colFlags = ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop;
            ...
            if (color != nullptr)
            {
                OpenPaletteColor(*color, typeColor);
#if EMSCRIPTEN
                saveColorSetting();
#endif
            }
    ...
    if (ImPlot::BeginPlot(PlotTiltle, ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y), ImPlotFlags_NoMenus))
    {
        ...
        for (size_t i = 0; i < m_chunck_process.size(); i++)
        {
            ...
            ImPlot::GetPlotDrawList()->AddRectFilled(ImPlot::PlotToPixels(ImPlotPoint(m_chunck_process[i].startTime, m_drawItems.coord[m_chunck_process[i].refIDProcess])),
                                                     ImPlot::PlotToPixels(ImPlotPoint(m_chunck_process[i].endTime, m_drawItems.coord[m_chunck_process[i].refIDProcess] - 0.2f)), Imv4ToCol32(theApp::Instance()->color_exec));
        }
        // std::cout<<std::endl;
    ...
    if (m_showTOP)
    {
        ImGui::BeginChild("##TOP", ImVec2(m_showSynopsis ? ImGui::GetContentRegionAvail().x / 2 : ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y), true, ImGuiWindowFlags_NoSavedSettings);
    ...
    if (m_showSynopsis)
    {
        ImGui::BeginChild("##Synopsis", ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y), true, ImGuiWindowFlags_NoSavedSettings);
        char data[255];
