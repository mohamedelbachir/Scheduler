...
void ScheduleTaskTab::badInputPopup()
{
    ...
    if (ImGui::BeginPopupModal(App::LangData("idWarning"), NULL, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text(App::LangData("idUserWrongInputContent"));
        ImGui::Separator();
        ImGui::Spacing();
        if (ImGui::Button(App::LangData("idbtnOk"), ImVec2(120, 0)))
        {
...
{
    if (m_generated)
        m_numberOfProcess = m_processes.size();
...
void ScheduleTaskTab::mainState()
{
    ...
    // left-side
    if (ImGui::BeginChild("left panel", ImVec2(250, 0), false, ImGuiWindowFlags_NoSavedSettings))
    {
        ImGui::Spacing();
        ...
        ImGui::RadioButton(App::LangData("idNoPremptive"), (int *)&m_groupSchedule, NOPREEMPTIF);
        ImGui::SameLine();
        ...
        ImGui::Spacing();
        ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4, 4));
        ImGui::Checkbox(App::LangData("idWithPriority"), &withPriority);
        ImGui::PopStyleVar();
        ImGui::Spacing();
        ImGui::BeginDisabled(m_generated);
        ImGui::Text(App::LangData("idLabelAlgoGenMode"));
        std::string algoGen = App::LangData("idDefault");
        algoGen += '\0';
        algoGen += App::LangData("idAlphabet");
        algoGen += '\0';
        algoGen += App::LangData("idAlphaNum");
        algoGen += '\0';
        algoGen += App::LangData("idNum");
        algoGen += '\0';
        ImGui::Combo("##gen mode", (int *)&m_algoGenType, algoGen.c_str());
        /*std::string algoGen[] = {
            App::LangData("idDefault"), App::LangData("idAlphabet"), App::LangData("idAlphaNum"), App::LangData("idNum")};
    ...
    // Right
    {
        ...
        ImGui::Spacing();
        ImGui::Spacing();
        ImGui::BeginChild("##table", ImVec2(tableZone), !m_generated, ImGuiWindowFlags_NoSavedSettings);
        if (m_generated)
        {
...
#else
        ImGui::OpenPopup(App::LangData("idWarning"));
#endif
...
void ScheduleTaskTab::showTableOfProcess(bool isEditable)
{
    ...
    static ImGuiTableFlags flags = ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_BordersV | ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_Resizable;
    if (isEditable)
        ImGui::PushStyleVar(ImGuiStyleVar_CellPadding, ImVec2(2, 2));
    else
        ImGui::PushStyleVar(ImGuiStyleVar_CellPadding, ImVec2(5, 6));
    if (ImGui::BeginTable("table_process", !withPriority ? 3 : 4, flags))
    {
        ...
        if (withPriority)
        {
            ImGui::TableSetupColumn(App::LangData("idTopPriority"), ImGuiTableColumnFlags_None);
        }
        if (!isEditable)
        {
        ...
        else
        {
            ...
            if (withPriority)
            {
                ...
                column_name = ImGui::TableGetColumnName(3); // Retrieve name passed to TableSetupColumn()
                ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 5);
                ImGui::SetCursorPosY(ImGui::GetCursorPosY() + minRowHeight / 2 - TEXT_BASE_HEIGHT / 2 + 5);
                ImGui::TableHeader(column_name);
            }
        ...
        for (size_t row = 0; row < m_processes.size(); row++)
        {
            ...
            if (isEditable)
            {
                ...
                if (m_processes[row].selected)
                {
                    if (it == m_process_checked.end())
                        m_process_checked.push_back(row);
                }
                else
                {
                    if (it != m_process_checked.end())
            ...
            if (withPriority)
            {
                ...
                if (isEditable)
                {
                    ...
                    sprintf(id, "##inputPriorityValue %d", ID + row);
                    ImGui::InputScalar(id, ImGuiDataType_U8, &m_processes[row].priority, &u8_one, NULL, "%u");
                    if (m_processes[row].priority < 1)
                    {
                        m_processes[row].priority = 1;
                    }
                    ImGui::PopStyleVar(3);
                    ImGui::PopItemWidth();
                }
                ...
                {
                    ImGui::Text("%d", m_processes[row].priority);
                }
            }
        }
        // ImGui::PopStyleVar();
