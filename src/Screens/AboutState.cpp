...
bool AboutState::onEnter()
{
    ...
    UI_Text *appVersion = new UI_Text(referenceWindow->getRenderer(), version.c_str(), appName->getVector()->getX(), appName->getVector()->getY() + appName->getHeight() + 10, "textInfo");

    UI_Text *appRelease = new UI_Text(referenceWindow->getRenderer(), "Copyright 2023", appVersion->getVector()->getX(), appVersion->getVector()->getY() + appVersion->getHeight() + 10, "textInfo");

    author = App::LangData("idAuthor");
    author += " : Mohamed El Bachir";
    authors = new UI_Text(referenceWindow->getRenderer(), author.c_str(), appRelease->getVector()->getX(), appRelease->getVector()->getY() + appRelease->getHeight() + 10, "textInfo");

    tools = App::LangData("idDevTools");
    tools += " : DearImGUI ( UI ) , SDL , Freetype";
    utools = new UI_Text(referenceWindow->getRenderer(), tools.c_str(), authors->getVector()->getX(), authors->getVector()->getY() + authors->getHeight() + 10, "textInfo");
    authors->onUpdate([&]
                      {
                        author=App::LangData("idAuthor");
                        author+=" : Mohamed El Bachir";
